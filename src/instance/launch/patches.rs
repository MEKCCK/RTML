...
const SHIM_JAR: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/rtml-shim.jar"));

fn deploy_shim(minecraft_dir: &Path) -> PathBuf {
    let dest = minecraft_dir.join(".rtml-shim.jar");
    if let Err(e) = std::fs::write(&dest, SHIM_JAR) {
        tracing::warn!("Failed to write rtml-shim.jar: {e}");
    }
    dest
}
...
fn write_log4j_config(minecraft_dir: &Path, jvm_args: &mut Vec<String>) {
    let config_path = minecraft_dir.join(".RTML-log4j2.xml");
