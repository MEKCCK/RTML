...
const DEFAULT_FILE_FILTER: &str = "warn,RTML=trace";
...
pub fn init() -> WorkerGuard {
    ...
    let file_appender = tracing_appender::rolling::never(&log_dir, format!("RTML_{now}.log"));
...
fn should_record_app_log(target: &str, level: Level) -> bool {
    ...
    target == "RTML" || target.starts_with("RTML::") || level <= Level::WARN
}
